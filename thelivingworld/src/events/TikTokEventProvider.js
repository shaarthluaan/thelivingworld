...
export class TikTokEventProvider {
 ...
 start(){if(this.socket||!this.socketFactory)return;this.state.status='CONNECTING';this.notify();try{const socket=this.socket=new this.socketFactory(this.url);socket.onopen=()=>{this.state.status='CONNECTED';this.state.error=null;this.notify()};socket.onmessage=e=>this.receive(e.data);socket.onerror=()=>{this.state.error='WebSocket bridge error';this.notify()};socket.onclose=()=>{this.socket=null;this.state.status='DISCONNECTED';this.notify()}}catch(err){this.state.status='ERROR';this.state.error=String(err);this.notify()}}
